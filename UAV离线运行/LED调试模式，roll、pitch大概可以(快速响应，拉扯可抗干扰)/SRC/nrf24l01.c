...
unsigned char NRF24L01_RxPacket(unsigned char *rxbuf)
{
	...
	{
		NRF24L01_Write_Reg(FLUSH_RX,0xff);delay();	//清除RX FIFO寄存器
	//	d=NRF24L01_Read_Reg(CD);delay();
	...
	if(sta&RX_OK)//发送完成
	{
		VAL=NRF24L01_Read_Reg(NRF_RD_RX_PL_WID);//0X60
...
unsigned char NRF24L01_TxPacket(unsigned char *txbuf)
{
	...
	NRF24L01_CE_L;delay();//要不要nRF24L01_Flush_TX_FIFO();// 清空 FIFO
  	NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,0x1A);//写数据到TX BUF  32个字节
...
{
  	NRF24L01_CE_L;
  	NRF24L01_Write_Buf(WRITE_REG+RX_ADDR_P0,(unsigned char*)RX_ADDRESS,RX_ADR_WIDTH);delay();//写RX节点地址

  	NRF24L01_Write_Reg(WRITE_REG+EN_AA,0x01);delay();    //使能通道0的自动应答
  	NRF24L01_Write_Reg(WRITE_REG+EN_RXADDR,0x01);delay();//使能通道0的接收地址
  	NRF24L01_Write_Reg(WRITE_REG+RF_CH,40);delay();	     //设置RF通信频率
  	NRF24L01_Write_Reg(WRITE_REG+RX_PW_P0,0x12);delay();//选择通道0的有效数据宽度
  	NRF24L01_Write_Reg(WRITE_REG+RF_SETUP,0x07);delay();//设置TX发射参数,0db增益,2Mbps,低噪声增益开启
  	NRF24L01_Write_Reg(WRITE_REG+CONFIG_24l01, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式
...
{														 
	NRF24L01_CE_L;	    
  	...
  	NRF24L01_Write_Reg(WRITE_REG+EN_AA,0x01);delay();     //使能通道0的自动应答
  	NRF24L01_Write_Reg(WRITE_REG+EN_RXADDR,0x01);delay(); //使能通道0的接收地址
  	NRF24L01_Write_Reg(WRITE_REG+SETUP_RETR,0x0f);delay();//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
...
{
	unsigned char buf[5]={0X5a,0X5a,0X5a,0X5a,0X5a};
	unsigned char i;
	NRF24L01_Write_Buf(WRITE_REG+TX_ADDR,buf,5);//写入5个字节的地址.
	delay_loop();
	NRF24L01_Read_Buf(TX_ADDR,buf1,5); //读出写入的地址
	for(i=0;i<5;i++)if(buf1[i]!=0X5a)break;
