...
float pressureAlt1=0;
Uint32 p=0;
Uint32 Pressure_AVG=0;
...
//        int32_t d1Average = 110;
    ...
    if(Pressure_AVG>=p)
    	pressureAlt1=(Pressure_AVG-p)*9.0f;//单位为cm       ((powf((1015.7f/p),0.1902630958f )-1.0f)*(25+273.15f))/0.0065f;
    else if(Pressure_AVG<p)
    	pressureAlt1=-((p-Pressure_AVG)*9.0f);
//    if(Pressure_AVG<p)
//    	pressureAlt1=0;
...
{
	static unsigned char filter_cnt2=0;
	float temp1=0;
	unsigned char i;
	Alt_BUF[filter_cnt2] = pressureAlt1;//更新滑动窗口数组
	for(i=0;i<smooth_tms;i++)
	{
		temp1 += Alt_BUF[filter_cnt2];
	}
	Alt_AVG = temp1 / (float)smooth_tms;
	filter_cnt2++;
	if(filter_cnt2==smooth_tms)	filter_cnt2=0;
}
...
{
	unsigned short ct=0;
	Uint32 Pressure_SUM=0;
	delay10ms();
	for(ct=0;ct<average_times;ct++)
	{
		readTemperatureRequestPressure();//读取MS5611温度
		delay10ms();
		readPressureRequestTemperature();
		delay10ms();
		Pressure_SUM+=p;
	}
	Pressure_AVG=Pressure_SUM/average_times;
}
