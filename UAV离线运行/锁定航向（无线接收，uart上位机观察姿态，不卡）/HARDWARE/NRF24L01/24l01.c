...
void NRF24L01_Init(void)
{
	...
	GPIOA->CRL&=0XFFFFFF0F;//PA1输出      CSN
	GPIOA->CRL|=0X00000030; 
	GPIOA->CRL&=0XFFFFF0FF;//PA2输出      CE
...
{
	u8 buf2[5]={0XA5,0XA5,0XA5,0XA5,0XA5};
	u8 i;
	SPIx_SetSpeed(SPI_SPEED_8); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   	 
	NRF24L01_Write_Buf(WRITE_REG+TX_ADDR,buf2,5);//写入5个字节的地址.	
...
//	  NRF24L01_Write_Buf(WRITE_REG+TX_ADDR,(u8*)TX_ADDRESS,TX_ADR_WIDTH);//写TX节点地址  
  	...
  	//NRF24L01_Write_Reg(RX_PW_P0,0x05);
	 // NRF24L01_Write_Reg(WRITE_REG+FLUSH_RX,0xff);//清除RX FIFO寄存器 
	 // NRF24L01_Write_Reg(WRITE_REG+STATUS,0x4e); //清除TX_DS或MAX_RT中断标志
...
{														 
	NRF24L01_CE=0;	    
  	...
  	NRF24L01_Write_Reg(WRITE_REG+EN_AA,0x00);     //使能通道0的自动应答    
  	NRF24L01_Write_Reg(WRITE_REG+EN_RXADDR,0x00); //使能通道0的接收地址  
