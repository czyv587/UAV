...
extern float Alt_AVG,Acc,Accdt11;
extern float Accdt,Accdt1,Accdt2;

void PID_INIT(void)	   
{
	Roll_core_kp=0.1;//((float)(BUF_recv[0]))/100;//0;//((float)(BUF_recv[0]));
	Roll_core_ki=0.01;//((float)(BUF_recv[1]))/1000;//0/100;//((float)(BUF_recv[1]))/100;
	Roll_core_kd=0.004;//((float)(BUF_recv[2]))/1000;//0;//((float)(BUF_recv[2]));

	Pitch_core_kp=0.1;//((float)(BUF_recv[3]))/100;//90;//((float)(BUF_recv[3]));
	Pitch_core_ki=0.01;//((float)(BUF_recv[4]))/1000;//30/100;//((float)(BUF_recv[4]))/100;
	Pitch_core_kd=0.004;//((float)(BUF_recv[5]))/1000;//20;//((float)(BUF_recv[5]));

	Yaw_core_kp=0.01;//((float)(BUF_recv[6]))/100;//((float)(BUF_recv[6]));
	Yaw_core_ki=0;//((float)(BUF_recv[7]))/1000;//((float)(BUF_recv[7]))/100;
	Yaw_core_kd=0;//((float)(BUF_recv[8]))/1000;//((float)(BUF_recv[8]));

	Roll_shell_kp=0.3;//((float)(BUF_recv[9]))/100;//0/100000;//((float)(BUF_recv[9]))/100000;
	Roll_shell_ki=0;//((float)(BUF_recv[10]))/100000;//0/1000000;//((float)(BUF_recv[10]))/1000000;


	Pitch_shell_kp=0.3;//((float)(BUF_recv[12]))/100;//80/100000;//((float)(BUF_recv[12]))/100000;
	Pitch_shell_ki=0;//((float)(BUF_recv[13]))/100000;//0;//((float)(BUF_recv[13]))/1000000;

	Yaw_shell_kp=0;//((float)(BUF_recv[15]))/100;//((float)(BUF_recv[15]))/100;
	Yaw_shell_ki=0;//((float)(BUF_recv[16]))/100000;//((float)(BUF_recv[16]))/100000;
...
//	Pitch_shell_out=RC_Pitch;
	...
	pitch_core_ki_out += Pitch_core_ki * (Pitch_shell_out - MPU6050_GYRO_LAST_Y);//限幅
	//-------------Pitch积分限幅----------------//
		if(pitch_core_ki_out>500) pitch_core_ki_out=500;
		else if(pitch_core_ki_out<-500) pitch_core_ki_out=-500;
	pitch_core_kd_out = -Pitch_core_kd * (MPU6050_GYRO_LAST_Y   - Gyro_radian_old_y);
...
//	Roll_shell_out=RC_Roll;
	...
	Roll_core_ki_out += Roll_core_ki * (Roll_shell_out - MPU6050_GYRO_LAST_X);//限幅
	//-------------Pitch积分限幅----------------//
		if(Roll_core_ki_out>500) Roll_core_ki_out=500;
		else if(Roll_core_ki_out<-500) Roll_core_ki_out=-500;
	Roll_core_kd_out = -Roll_core_kd * (MPU6050_GYRO_LAST_X - Gyro_radian_old_x);
	...
	Yaw_core_ki_out += Yaw_core_ki * (Yaw_shell_out - MPU6050_GYRO_LAST_Z);// * 3.5
		if(Yaw_core_ki_out>500) Yaw_core_ki_out=500;
		else if(Yaw_core_ki_out<-500) Yaw_core_ki_out=-500;
...
//	Motor1=(int)(pwm_num3 + Roll_core_out - Pitch_core_out);//- pitch + Roll - yaw
	/*Motor1=(int)(Acc*100);//Alt_AVG 0     //- pitch - Roll + yaw//MPU6050_GYRO_LAST_Z;
	Motor2=(int)(Alt_AVG);//Pitch_shell_out;//Yaw;
	Motor3=(int)(Accdt2);//RC_Pitch;//RC_Roll;*/
	Motor1=(int)(pwm_num3 + Roll_core_out -  Pitch_core_out);
	Motor2=(int)(pwm_num3 - Roll_core_out + Pitch_core_out);//+ pitch - Roll - yaw
	Motor3=(int)(pwm_num3 + Roll_core_out + Pitch_core_out);//+ pitch + Roll + yaw
	//Yaw_core_out=0;
