...
void PID_INIT(void)	   
{
	Roll_core_kp=((float)(20));//0;//((float)(BUF_recv[0]));//
	Roll_core_ki=((float)(10))/100;//0/100;//((float)(BUF_recv[1]))/100;//
	Roll_core_kd=((float)(10));//0;//((float)(BUF_recv[2]));//

	Pitch_core_kp=((float)(20));//90;//((float)(BUF_recv[3]));//
	Pitch_core_ki=((float)(10))/100;//30/100;//((float)(BUF_recv[4]))/100;//
	Pitch_core_kd=((float)(10));//20;//((float)(BUF_recv[5]));//

	Roll_shell_kp=((float)(42))/100;//0/100000;//((float)(BUF_recv[9]))/10;//
	Roll_shell_ki=((float)(0))/1000;//0/1000000;//((float)(BUF_recv[10]))/1000;//


	Pitch_shell_kp=((float)(42))/100;//80/100000;//((float)(BUF_recv[12]))/10;//
	Pitch_shell_ki=((float)(0))/1000;//0;//((float)(BUF_recv[13]))/1000;//
...
void control2(void)
{
	...
	if((Time2on>228148)&&(Time2on<300000))
		RC_Pitch=-((float)(Time2on-228148))/(1804/2);//(Rc_D.ROLL-1500)/20;
	else if((Time2on<228148)&&(Time2on>160000))
		RC_Pitch=(((float)(228148-Time2on))/(1804/2));
	//RC_Pitch=0;//(Rc_D.PITCH-1500)/20;
////////////////////////外环角度环(PID)///////////////////////////////
	Pitch_i+=(RC_Pitch-Pitch);
//-------------Pitch积分限幅----------------//
...
/*********************************************************/
	if((Time1on>229253)&&(Time1on<300000))
		RC_Roll=((float)(Time1on-229253))/(1966/2);//(Rc_D.ROLL-1500)/20;
	else if((Time1on<229253)&&(Time1on>160000))
		RC_Roll=-(((float)(229253-Time1on))/(1966/2));
//	RC_Roll=0;
	Roll_i+=(RC_Roll-Roll);
//-------------Roll积分限幅----------------//
	Roll_shell_ki_out=Roll_shell_ki*Roll_i;
...
//	Pitch_shell_out=RC_Pitch;
	...
	pitch_core_ki_out += Pitch_core_ki * (Pitch_shell_out - MPU6050_GYRO_LAST_Y);//限幅
	//-------------Pitch积分限幅----------------//
		if(pitch_core_ki_out>3000) pitch_core_ki_out=3000;
		else if(pitch_core_ki_out<-3000) pitch_core_ki_out=-3000;
	pitch_core_kd_out = 0.2*(-Pitch_core_kd * (MPU6050_GYRO_LAST_Y   - Gyro_radian_old_y))+0.8*pitch_core_kd_out;

//	Roll_shell_out=RC_Roll;
	Roll_core_kp_out = Roll_core_kp * (Roll_shell_out  - MPU6050_GYRO_LAST_X);
	Roll_core_ki_out += Roll_core_ki * (Roll_shell_out - MPU6050_GYRO_LAST_X);//限幅
	//-------------Pitch积分限幅----------------//
		if(Roll_core_ki_out>3000) Roll_core_ki_out=3000;
		else if(Roll_core_ki_out<-3000) Roll_core_ki_out=-3000;
	Roll_core_kd_out = 0.2*(-Roll_core_kd * (MPU6050_GYRO_LAST_X - Gyro_radian_old_x))+0.8*Roll_core_kd_out;
